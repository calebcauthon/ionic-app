// Ionic Starter App

// angular.module is a global place for creating, registering and retrieving Angular modules
// 'starter' is the name of this angular module example (also set in a <body> attribute in index.html)
// the 2nd parameter is an array of 'requires'
angular.module('starter', ['ionic'])

.controller('addRecipeCtrl', addRecipeCtrl)
.directive('eggTimer', eggTimer)
  .controller('eggTimerCtrl', eggTimerCtrl)
.directive('recipeList', recipeList)
  .controller('recipeListCtrl', recipeListCtrl)
.directive('recipe', recipe)
  .controller('recipeCtrl', recipeCtrl)
.directive('commandEntry', commandEntry)
  .controller('commandEntryCtrl', commandEntryCtrl)
.service('houndify', houndify)
.service('chosenRecipe', chosenRecipe)
.service('spaghetti', spaghetti)
.service('ux', ux)
.service('speak', speak)
.service('commands', commands)

.constant('SERVER', {
  // Local server
  //url: 'http://localhost:3000'

  // Public Heroku server
  url: 'https://localhost:3446'
})

.run(function($ionicPlatform) {
  $ionicPlatform.ready(function() {
    if(window.cordova && window.cordova.plugins.Keyboard) {
      // Hide the accessory bar by default (remove this to show the accessory bar above the keyboard
      // for form inputs)
      cordova.plugins.Keyboard.hideKeyboardAccessoryBar(true);

      // Don't remove this line unless you know what you are doing. It stops the viewport
      // from snapping when text inputs are focused. Ionic handles this internally for
      // a much nicer keyboard experience.
      cordova.plugins.Keyboard.disableScroll(true);
    }
    if(window.StatusBar) {
      StatusBar.styleDefault();
    }
  });
})

.config(function($stateProvider, $urlRouterProvider) {
  $stateProvider
  .state('recipe', {
    url: '/recipe',
    templateUrl: 'templates/recipe.html',
    controller: function() {}
  });

  $urlRouterProvider.otherwise('/recipe');
})
